...
import { PortfolioItemInputSchema, LotSchema, SellSchema } from '../validation/schemas';
...
export const sellFromItem = (req: Request, res: Response): void => {
  try {
    const { isin } = req.params;
    ...
    loggerService.error('Error selling from item', error as Error);
    res.status(500).json({ success: false, message: 'Server Error' });
  }
};
